    ...
    module GroupAccess
      ...
      included do
        ...
        def set_user_group!(user_group, yes_no=true, scope=nil)
          if ::Universal::Configuration.scoped_user_groups and !scope.nil?
            user_group = ::Universal::UserGroup.where(scope: scope).find(user_group)||::Universal::UserGroup.find_by(scope: scope, code: user_group) if user_group.class == String
          else
            user_group = ::Universal::UserGroup.find(user_group)||::Universal::UserGroup.find_by(code: user_group) if user_group.class == String
          end
          if !user_group.nil?
            if self._ugid.nil?
        ...
        def update_user_group_functions!
          all_functions = {}
          user_groups = ::Universal::UserGroup.in(id: self._ugid)
        ...
        def universal_user_groups(scope=nil)
          ...
          if ::Universal::Configuration.scoped_user_groups
            @user_groups ||= ::Universal::UserGroup.in(id: self._ugid, scope: scope).cache
          else
            @user_groups ||= ::Universal::UserGroup.in(id: self._ugid).cache
          end
        end
        ...
        def has?(category, function=nil, scope=nil)
          ...
          if ::Universal::Configuration.scoped_user_groups
            raise 'Nil Scope. A scope model must be passed to has? method in Universal::Concerns::GroupAccess' if scope.nil?
